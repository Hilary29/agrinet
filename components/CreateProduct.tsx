  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get<Category[]>(
        ressourcesRoutes.ressourcesCategorie
      );
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const response = await axios.post(
        ressourcesRoutes.ressourcesProductPostCreate,
        formData,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      if (selectedFiles.length > 0) {
        ...
        await axios.post(
          `${ressourcesRoutes.ressourcesMediaAddProduct}/${productResponse.data.id}`,
          formData,
          {
            headers: {
      ...
      <form onSubmit={handleSubmit} className="space-y-1">
        ...
        <div className="flex flex-row gap-8 justify-between">
          ...
          <button
            ...
            className="w-[30%] bg-green-600 text-white-50 py-2 rounded hover:bg-green-700"
          >
            + New Category
          </button>
        </div>
...
function MediaUpload({ productId }: MediaUploadProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        `${ressourcesRoutes.ressourcesMediaAddProduct}/${productId}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  return (
    <div className="mt-4">
      <h3 className="text-lg font-semibold mb-2">Add Medias</h3>
      <input
        type="file"
        multiple
        onChange={handleFileChange}
