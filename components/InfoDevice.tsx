...
const InfoDevice: React.FC<InfoModalProps> = ({ isOpen, onClose, device }) => {
  ...
  const renderGraph = () => {
    ...
    switch (device.type) {
      ...
      default:
        return <div className="text-center text-gray-500">No graph available for this device type.</div>; // Fallback message
    }
  };
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="md:w-3/4 p-4 flex flex-col">
            <div className="flex-grow overflow-hidden">
              <div className="h-full w-full">{renderGraph()}</div>
            </div>
          </div>
        </div>
      </div>
